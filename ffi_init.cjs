const { Tensor, bytesUsed } = require('./ffi_impl/js/index.cjs');
...
const runTest = (runs) => {
  ...
  console.log(`avg time (${runs} runs) to init 10k Tensors of 1k elements (dtype = Float64; ${runs} runs): ${times.reduce((a, b) => a + b, 0) / runs} seconds`)
